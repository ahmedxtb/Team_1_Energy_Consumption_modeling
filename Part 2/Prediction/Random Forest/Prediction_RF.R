...
for(b in 1:length(list_of_buildings_heat)){
  ...
  rf_fit <- randomForest(y~ train$WindDirDegrees + train$Base_hour_Flag +train$Weekday + train$month      
                         +train$TemperatureF +train$Dew_PointF ,data=train,ntree = n_tree_value)
...
write.csv(final_file,file="RF_Pred_models.csv",row.names=FALSE)
write.csv(outlier_tag_file,file = "Outlier_RF_Pred_models.csv")
